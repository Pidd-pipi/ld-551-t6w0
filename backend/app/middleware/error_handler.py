...
class ErrorHandlerMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        except (AuthException, PermissionDeniedException, CoursePermissionException, NoteAccessDeniedException) as exc:
            return JSONResponse(status_code=403, content={"code": 403, "message": exc.message, "detail": None})
        except NoteConflictException as exc:
            return JSONResponse(status_code=409, content={"code": 409, "message": exc.message, "detail": {"current_version": exc.current_version}})
